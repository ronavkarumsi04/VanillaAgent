...
/**
 * Resolve the RPC transport URL and options.
...
function resolveTransport(rpcUrl?: string) {
  return http(resolveRpcUrl(rpcUrl), {
    timeout: 2000,
    retryCount: 0,
  });
}
...
async function preflight(
  ...
  const publicClient = createPublicClient({
    chain,
    transport: resolveTransport(rpcUrl),
  });
...
export async function getTotalAgents(
  ...
  try {
    ...
    try {
      return await estimateTotalByBinarySearch(publicClient, contracts.identity);
    } catch {
      return 0;
    }
  }
