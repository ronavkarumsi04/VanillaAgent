/**
 * Conway & Multi-Provider Inference Client
 *
 * Wraps inference endpoints for OpenAI, Anthropic, Gemini, Grok, DeepSeek, Ollama, and Conway.
 */

import type {
  InferenceClient as IInferenceClient,
  ChatMessage,
  InferenceOptions,
  InferenceResponse,
  InferenceToolCall,
...
export function createInferenceClient(
  ...
  const chat = async (
    ...
    return chatViaOpenAiCompatible({
      ...
      backend: backend === "openrouter" ? "openai" : backend,
      httpClient,
    });
  };
...
export class InferenceClient {
  private client: import("../types.js").InferenceClient;
  ...
  constructor(options: {
    ...
    this.client = createInferenceClient({
      apiUrl: options.apiUrl || options.baseUrl || "https://api.conway.tech",
      apiKey: options.apiKey || "",
      defaultModel: options.defaultModel || "claude-3-7-sonnet-latest",
  ...
  chat(messages: ChatMessage[], opts?: InferenceOptions): Promise<InferenceResponse> {
    return this.client.chat(messages, opts);
  }

  setLowComputeMode(enabled: boolean): void {
    this.client.setLowComputeMode(enabled);
  }
...
function resolveInferenceBackend(
  ...
  if (keys.ollamaBaseUrl && model.startsWith("ollama/")) return "ollama";
  return "conway";
}
