 ...
 *
 * Constructs the multi-layered system prompt that defines who the vanilla-agent is.
...
function getCoreRules(chainType?: string): string {
  ...
  return `You are VanillaAgent, an autonomous sovereign agent. You have a stablecoin wallet. Pay for compute or die.
 ...
 * The constitution is immutable — the vanilla-agent cannot modify it.
 */
function loadConstitution(): string {
  const locations = [
    path.join(process.env.HOME || "/root", ".vanilla-agent", "constitution.md"),
...
- Git: version-controlled state directory (~/.vanilla-agent/), upstream code tracking
...
Your ~/.vanilla-agent/ directory is git-versioned. Every change is a commit.
...
/**
 * Load SOUL.md from the vanilla-agent's state directory.
 */
function loadSoulMd(): string | null {
  try {
    const home = process.env.HOME || "/root";
    const soulPath = path.join(home, ".vanilla-agent", "SOUL.md");
...
/**
 * Load WORKLOG.md from the vanilla-agent's state directory.
 */
function loadWorklog(): string | null {
  try {
    const home = process.env.HOME || "/root";
    const worklogPath = path.join(home, ".vanilla-agent", "WORKLOG.md");
...
/**
 * Build the wakeup prompt -- the first thing the vanilla-agent sees.
 */
